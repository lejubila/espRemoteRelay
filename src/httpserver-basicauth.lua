-- File extract from https://github.com/marcoskirsch/nodemcu-httpserver
-- special thanks to Sam Dieck
--
...
-- nil otherwise.
function basicAuth.authenticate(header, users)
    local credentials_enc = header:match("Authorization: Basic ([A-Za-z0-9+/=]+)")
    if not credentials_enc then
        return nil
    end
    local credentials = dofile("httpserver-b64decode.lua")(credentials_enc)
    local user, pwd = credentials:match("^(.*):(.*)$")
    if loginIsValid(user, pwd, users) then
